...
//  This is MUCH faster than Python's grim -> PNG -> PIL decode -> resize -> JPEG
// ========================================================================
...
std::vector<uint8_t> VideoStreamer::capture_and_encode() {
    ...
    if (status != 0 || ppm_data.size() < 16) return {};
    ...
    while (pos < ppm_data.size() && ppm_data[pos] >= '0' && ppm_data[pos] <= '9') {
    ...
    for (int y = 0; y < stream_height_; ++y) {
        ...
        if (src_y >= cap_height) src_y = cap_height - 1;
        for (int x = 0; x < stream_width_; ++x) {
            int src_x = (int)(x * x_ratio);
            if (src_x >= cap_width) src_x = cap_width - 1;
...
void VideoStreamer::start_streaming() {
    ...
    printf("[*] Stream resolution: %dx%d, JPEG quality: %d\n",
           stream_width_, stream_height_, jpeg_quality_);
    ...
    while (g_running) {
        ...
        bool ok = client_.send_video_frame(
            0, 0, stream_width_, stream_height_,
            jpeg.data(), jpeg.size());
