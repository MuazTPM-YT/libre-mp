...
class VideoStreamer:
    ...
    def start_streaming(self):
        ...
        # 2. Encode to exact dynamic-sized JPEG
        import io
        buf = io.BytesIO()
        frame.save(buf, format="JPEG", quality=self.jpeg_quality, subsampling=2, optimize=False, progressive=False)
        jpeg_bytes = buf.getvalue()
        
        # 3. Fire the single frame down the VIDEO socket
        print(f"[*] Firing single dynamic-sized frame ({len(jpeg_bytes)} bytes of JPEG data)...")
        success = self.client.send_video_frame(0, 0, self.stream_width, self.stream_height, jpeg_bytes)
        
        if success:
            print("\n[+] BINGO! Frame accepted by the projector without crashing!")
            print("[*] Holding TCP connection open for 15 seconds...")
            print("[!] Tell your friend to look at the wall NOW!")
            
            # Keep the connection alive so the projector doesn't clear the screen
            import time
            for i in range(15):
                # Send the keepalive every second so the projector doesn't drop us
                self.client._send_frame_keepalive()
                time.sleep(1)
                
            print("\n[*] Isolation test complete. Disconnecting gracefully.")
        else:
            print("[-] Failed to send the frame.")